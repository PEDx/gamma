...
function EditableBox({ adsorbLineArr, onChange, onMouseDown }, ref) {
  let x0, y0, x1, y1;
  let L0, R0, T0, B0, EH, EW;
  let offsetRight,
    offsetBottom = 0;
  const adsorbLineX = useRef(0);
  const adsorbLineY = useRef(0);
  const isEditing = useRef(false);
  const isMoving = useRef(false);
  const active = useRef(null);
  const editableDataView = useRef(null);
  const editableElement = useRef(null);
  const editableBox = useRef(null);
  const adsorb_x_arr = useRef([]);
  const adsorb_y_arr = useRef([]);
  const editDirections = useRef(DIRECTIONS.NULL);
  ...
  const move_mousemoveHandler = useCallback((e) => {
    ...
    if (X > adsorb_x - EW / 2 - distance && X < adsorb_x - EW / 2 + distance) {
      X = adsorb_x - EW / 2;
      adsorbLineX.current = X;
    }
    ...
    if (Y > adsorb_y - EH / 2 - distance && Y < adsorb_y - EH / 2 + distance) {
      Y = adsorb_y - EH / 2;
      adsorbLineY.current = Y;
    }
  ...
  const updateEditableBoxAttr = useCallback((key, value) => {
...
export default forwardRef(EditableBox);
